...
function transform(data) {
  ...
  for (const thread of data.threads) {
    ...
    const entry = {
      ...
      type: "thread",
      commit: null,
      // Will be filled from first comment's review comment
      path: thread.path,
      line: thread.line,
    ...
    if (thread.comments.length > 0) {
      ...
      const numericId = firstCommentId.includes("_") ? void 0 : parseInt(firstCommentId, 10);
      if (numericId) {
...
function isFullwidth(code) {
  return code >= 4352 && code <= 4447 || code >= 11904 && code <= 12350 || code >= 12352 && code <= 12447 || code >= 12448 && code <= 12543 || code >= 13312 && code <= 19903 || code >= 19968 && code <= 40959 || code >= 44032 && code <= 55215 || code >= 63744 && code <= 64255 || code >= 65040 && code <= 65049 || code >= 65072 && code <= 65135 || code >= 65281 && code <= 65376 || code >= 65504 && code <= 65510 || code >= 131072 && code <= 196605 || code >= 196608 && code <= 262141;
}
...
function typeLabel(type) {
  switch (type) {
    case "thread":
      return source_default.bold.magenta("THREAD ");
    case "review":
      return source_default.bold.cyan("REVIEW ");
    case "issue_comment":
      return source_default.bold.blue("COMMENT");
    default:
      return source_default.bold(type.toUpperCase());
  }
}
function prStateStyle(state) {
  switch (state) {
    case "OPEN":
      return source_default.bgGreen.black.bold(` ${state} `);
    case "MERGED":
      return source_default.bgMagenta.white.bold(` ${state} `);
    case "CLOSED":
      return source_default.bgRed.white.bold(` ${state} `);
    default:
      return source_default.bold(state);
  }
}
function reviewStateStyle(state) {
  switch (state) {
    case "APPROVED":
      return source_default.green.bold(state);
    case "CHANGES_REQUESTED":
      return source_default.red.bold(state);
    case "COMMENTED":
      return source_default.yellow(state);
    case "DISMISSED":
      return source_default.dim(state);
    default:
      return state;
  }
}
...
function drawBox(lines) {
  ...
  const inner = width - 2;
  const top = source_default.dim(`  \u256D${"\u2500".repeat(inner)}\u256E`);
  const bottom = source_default.dim(`  \u2570${"\u2500".repeat(inner)}\u256F`);
  const empty = source_default.dim("  \u2502") + " ".repeat(inner) + source_default.dim("\u2502");
  const contentLines = lines.map((line) => {
    const pad = inner - 2 - stringWidth(line);
    return source_default.dim("  \u2502") + `  ${line}` + " ".repeat(Math.max(0, pad)) + source_default.dim("\u2502");
  });
...
function sectionHeader(title, count) {
  ...
  return source_default.dim(`  \u2500\u2500 ${source_default.bold.white(label)} ${line}`);
}
function stripHtml(text) {
  return text.replace(/<!--[\s\S]*?-->/g, "").replace(/<details>[\s\S]*?<\/details>/g, "").replace(/<[^>]+>/g, "").replace(/&nbsp;/g, " ").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/\n{3,}/g, "\n\n").replace(/^\s*\n/gm, "\n");
}
function extractSummary(body, maxLen = 72) {
...
function renderMarkdownBody(body) {
  ...
  text = text.replace(
    /```suggestion[\s\S]*?```/g,
    source_default.dim.italic("  [suggestion: see GitHub for diff]")
  );
  text = text.replace(/```(\w*)\n([\s\S]*?)```/g, (_m, _lang, code) => {
    return code.split("\n").map((line) => source_default.dim(`    ${line}`)).join("\n");
  });
  text = text.replace(/^---+$/gm, source_default.dim("\u2500".repeat(40)));
  text = text.replace(/`([^`]+)`/g, (_m, code) => source_default.cyan(code));
  text = text.replace(/\*\*(.+?)\*\*/g, (_m, content) => source_default.bold(content));
  text = text.replace(
    /(?<!\*)\*([^*]+)\*(?!\*)/g,
    (_m, content) => source_default.italic(content)
  );
...
function formatDate(dateStr) {
  return source_default.dim(dateStr.substring(0, 10));
}
function formatDateTime(dateStr) {
  return source_default.dim(dateStr.substring(0, 16).replace("T", " "));
}

// src/viewer/list.ts
var ACTION_ORDER = ["pending", "fix", "skip", "done"];
function renderList(entries, meta) {
  const lines = [];
  lines.push(
    drawBox([
      `${source_default.bold.cyan(`PR #${meta.number}`)}  ${prStateStyle(meta.state)}`,
      source_default.bold(meta.title),
      source_default.dim(`${meta.baseRefName} \u2190 ${meta.headRefName}`)
    ])
  );
  lines.push("");
  ...
  const summary = ACTION_ORDER.map((a) => {
    ...
    const count = source_default.bold(String(counts.get(a)));
...
function renderCommentCard(entry) {
  const lines = [];
  const color = actionStyle(entry.action);
  const bar = color("\u2503");
  const header = `  ${typeLabel(entry.type)}  ${actionIcon(entry.action)} ${actionLabel(entry.action)}`;
...
// src/viewer/detail.ts
function renderDetail(entry, _meta) {
  const lines = [];
  switch (entry.type) {
    case "thread":
...
function renderThreadDetail(entry) {
  const lines = [];
  const color = actionStyle(entry.action);
  const bar = color("\u2503");
  const boxLines = [
    `${typeLabel(entry.type)}  ${actionIcon(entry.action)} ${actionLabel(entry.action)}`,
    source_default.dim(entry.id)
  ];
  if (entry.path) {
    const loc = entry.line ? `${entry.path}:${entry.line}` : entry.path;
    boxLines.push(source_default.underline.cyan(loc));
  }
  boxLines.push(
    entry.is_resolved ? source_default.green("Resolved: Yes") : source_default.yellow("Resolved: No")
  );
  lines.push(drawBox(boxLines));
  lines.push("");
  for (const comment of entry.comments) {
    const author = comment.author ? source_default.bold(`@${comment.author}`) : source_default.dim("(unknown)");
...
function renderReviewDetail(entry) {
  ...
  if (entry.commit) {
    boxLines.push(source_default.dim(`commit ${entry.commit.substring(0, 12)}`));
  }
  lines.push(drawBox(boxLines));
  lines.push("");
  if (entry.body) {
...
function renderCommentDetail(entry) {
  const lines = [];
  const color = actionStyle(entry.action);
  const bar = color("\u2503");
  const author = entry.author ? source_default.bold(`@${entry.author}`) : source_default.dim("(unknown)");
...
// src/viewer/index.ts
function viewCommand(owner, repo, prNumber, entryId, options) {
...
function resolvePRIdentifier(pr, repoOption) {
  if (pr) {
...
program2.command("view [pr] [entry-id]").description("Display synced PR review data in a human-readable format").option("-o, --output <dir>", "Output directory", "/tmp/github.com").option(
  "-R, --repo <repo>",
  "Repository in OWNER/REPO format (auto-detected from cwd)"
